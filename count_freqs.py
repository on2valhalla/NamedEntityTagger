...
class Hmm(object):
    ...
    def emission_probability(self, word, tag):
        if (word, tag) in self.emission_counts:  # pair has a valid emission count
            return self.emission_counts[(word, ne_tag)] / self.ngram_counts[1][tag]
        elif ne_tag in self.all_states:  # no valid emission count, treat word as rare
            return self.emission_counts[(self.rare_symbol, ne_tag)] / self.ngram_counts[1][tag]
